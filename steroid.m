function [V,l,e,varargout]=steroid(A,varargin)
% [V,l,e,tail,lambdas]=steroid(A,method) or [V,tail]=steroid(A,method)
% --------------------------------------------------------------------
...
%               contributions in the STEROID. Optional output argument,
%
% lambdas   =   vector, contains the weights in the STEROID. Optional output
%               argument,
%
...
%         lstime=toc;
        ...
        if nargout > 3
            varargout{1}=reshape(a-W*lambdas,n*ones(1,doriginal));  % compute symmetric tail
            varargout{2}=lambdas;
        else
            varargout=cell(1,1);
        end
